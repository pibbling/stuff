...
# Set the display name explicitly to HDMI-A-1
...
apply_custom_resolution() {
  ...
  MODELINE=$(cvt  ${RESOLUTION%x*} ${RESOLUTION#*x} $REFRESH_RATE | grep Modeline | cut -d' ' -f2-)
...
if [[ $choice == "1" ]]; then
  ...
  apply_custom_resolution "1440x1080" "239"
elif [[ $choice == "2" ]]; then
